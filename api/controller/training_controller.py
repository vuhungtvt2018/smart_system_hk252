...
async def upload_test_dataset(file: UploadFile = File(...)):
    ...
    try:
        file_path = await TrainingService.save_test_dataset(file)
        return {"status": "success", "message": "Test file uploaded successfully", "dataset_path": file_path}
    except Exception as e:
...
def check_test_dataset():
    """
    Returns whether a test dataset exists in the directory.
    """
    exists = TrainingService.has_test_dataset()
